...
class MolerAdhocDataset(Dataset):
    def __init__(
        self,
        root,
        raw_moler_trace_dataset_parent_folder,  # absolute path
    ...
    def _save_processed_gen_step(self, pkl_file_path_molecule_idx_molecule_gen_steps):
        ...
        # file_name = f'{pkl_file_path.split("/")[-1].split(".")[0]}_mol_{molecule_idx}.pkl.gz'  #
        # file_path = os.path.join(
        #     self._output_pyg_trace_dataset_parent_folder,
        #     self._split,
        #     file_name,
        # )
        # with gzip.open(file_path, "wb") as step_file_path:
        #     pickle.dump(molecule_gen_steps, step_file_path)
